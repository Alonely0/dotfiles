...
-- Configuration of window overview
sweetColorizer = colorRangeFromClassName
                     (0x70, 0xFF, 0x70) -- lowest inactive bg
                     (0x70, 0xFF, 0x70) -- highest inactive bg
                     (0x30, 0x37, 0x51) -- active bg
                     (0xFF, 0xFF, 0xFF) -- inactive fg
                     (0xFF, 0xFF, 0xFF) -- active fg

gsconfig sweetColorizer = (buildDefaultGSConfig sweetColorizer) { gs_cellheight = 120, gs_cellwidth = 300 }
...
--
myNormalBorderColor  = "#181b28"
myFocusedBorderColor = "#303751"
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- Show all windows
    , ((modm,               xK_g), goToSelected $ gsconfig sweetColorizer)

    -- lock screen
    , ((modm,               xK_F1    ), spawn "loginctl lock-session")
...
myManageHook = fullscreenManageHook <+> manageDocks <+> composeAll
    ...
    , resource  =? "Plasma"         --> doIgnore
    , className =? "Latte Dock"     --> doFloat
    , resource  =? "Latte Dock"     --> doFloat
